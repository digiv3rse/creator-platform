...
import { WnftArgs, Accent } from "transform";

import { ObjectEntries } from "./util";

function getStringOrNull(args: {
  value: string;
  transformArg: keyof WnftArgs;
}): string | null {
...
export function Wnft(props: WnftArgs & { user: string }) {
  const [transformArgs, setWnftArgs] = React.useState<WnftArgs>({
    theme: props.theme,
    title: props.title,
    featuredImageUrl: props.featuredImageUrl,
    avatarUrl: props.avatarUrl,
    user: props.displayUser,
  ...
  const imageUrl = React.useMemo(() => {
    ...
    for (const [key, value] of Object.entries(transformArgs) as ObjectEntries<
      typeof transformArgs
    >) {
      if (value !== null) {
    ...
    return `/api/transform?${params.toString()}` as const;
  }, [transformArgs]);
                  ...
                  return (
                    <label key={themeValue} style={{ display: "flex", gap: 3 }}>
                      <input
                        type="radio"
                        value={themeValue}
                        checked={transformArgs.theme === themeValue}
                        onChange={() => {
                          setWnftArgs((prev) => ({
                            ...prev,
                            theme: themeValue,
                  ...
                  return (
                    ...
                    >
                      <input
                        type="radio"
                        value={accentValue}
                        checked={transformArgs.accent === accentValue}
                        onChange={() => {
                          setWnftArgs((prev) => ({
                            ...prev,
                            accent: accentValue,
